...
- `SELLLIMIT`: 是指相对于现价而言,以**高于现价的价格挂单卖出**操作指令

`SELLLIMIT`是设定为以更高的价格卖出(做空).假设现在`EUR:USD = 1: 1.4321`,你认为趋势仍然是向下的,但是价格会反弹到`EUR:USD = 1:1.4350`,
然后还会继续下跌.这种情况下可以在价格为`EUR:USD = 1:1.4350`处挂一张`SELLLIMIT`挂单,当价格从`EUR:USD = 1: 1.4321`上涨到`EUR:USD = 1: 1.4350`
时,卖单将会成交.若价格下跌,则你会盈利.
...
void placeSellSELLLIMITOrder() {
   ...
   for(int pendingOrderLineIndex = 1; pendingOrderLineIndex <= pendingOrderLineNumber; pendingOrderLineIndex++) {
      ...
      /*
      Print("这是SELLLIMIT的第 ", pendingOrderLineIndex, " 单");
...
| `SELLLIMIT`订单顺序 | 市价(`BID`价格) |  下单价格   |  订单止盈价  |  订单止损价  |
|:---------------:|:-----------:|:-------:|:-------:|:-------:|
|        1        |   174.046   | 174.246 | 173.946 | 174.346 |
|        2        |   174.046   | 174.446 | 174.146 | 174.546 |
|        3        |   174.046   | 174.646 | 174.346 | 174.746 |
